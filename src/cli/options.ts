...
export function addImageOptions(cmd: Command): Command {
  return cmd
    ...
    .option("--style <style>", "Map style: topo, terrain, liberty, bright, positron, outdoor, dark, light, satellite, mapbox-outdoor, mapbox-satellite, mapbox-standard, mapbox-dark, mapbox-light", "topo")
    ...
    .option("--mapbox-token <token>", "Mapbox access token (required for mapbox-* and satellite styles)")
...
export function addVideoOptions(cmd: Command): Command {
  return cmd
    ...
    .option("--dpi <scale>", "Device pixel ratio", "1")
